...
- Phase 2 visual foundation with procedural Aurora Ribbon flow, radial speed
  lines, bounded boost/impact chromatic pulses, and pooled instanced FX
- Phase 2 horizon pass with layered procedural nebula depth and deterministic
  landmark anchors that scale down by quality tier
- Normalized analyzer bands feeding bounded track, aurora, color-grade, and
  rival-trail presentation without changing audio output or race state

No downloaded assets are used. Meshes, shaders, UI, and sound are generated by
the codebase.

Phase 2 visual decisions and vocabulary live in `CONTEXT.md` and
`docs/adr/0001-phase-2-visual-architecture.md`.
